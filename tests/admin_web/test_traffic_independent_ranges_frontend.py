...
(async()=>{
  ...
  assert(queued.at(-1).options.notBefore===4000,"guard starts at actual dispatch");
  ...
  assert(queued.at(-1).options.notBefore===4000,"explicit selection cannot bypass guard");
...
element("admin-page").dataset={page:"traffic",siteId:site,apiBase:`/admin/api/v1/sites/${site}`,trafficEnabled:"true",trafficIndependentRangesEnabled:"true",trafficRefreshSeconds:"60",trafficRequestTimeoutSeconds:"30"};
element("traffic-history-panel").dataset.historyEnabled="true";
element("traffic-statistics-panel").dataset.statisticsEnabled="true";
element("traffic-peak-panel").dataset.peakEnabled="true";
element("traffic-ap-panel").dataset.apEnabled="true";
...
(async()=>{
  ...
  now=1000;document.hidden=true;(documentListeners.visibilitychange||[]).forEach((callback)=>callback());
  await settle();await settle();
  ...
  now=2000;document.hidden=false;(documentListeners.visibilitychange||[]).forEach((callback)=>callback());
  await settle();await settle();
  assert(dispatches===1,"visibility resume cannot bypass T+3 admission");
  assert(element("traffic-history-state-title").textContent.includes("waiting"),"resume does not fake loading");

  now=2999;fireNextTimer();await settle();await settle();
  assert(dispatches===1,"restored request is not admitted before T+3");
  now=3000;fireNextTimer();await settle();await settle();
  assert(dispatches===2&&active===1,"exactly one restored request is admitted at T+3");
...
def test_independent_ranges_keep_network_and_lifecycle_out_of_panel_broker():
    ...
    assert "ADMISSION_GUARD_MILLISECONDS = 3000" in independent
